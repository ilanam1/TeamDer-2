...
def send_message():
    # לשלוף את שם המשתמש וההודעה מהטופס
    username = request.form['username']
    message = request.form['message']

    # לשמור את הקובץ שהועלה בטופס
    if 'file' in request.files:
        file = request.files['file']
        # בדיקה שהקובץ קיים ושהוא לא ריק
        if file.filename != '' and file:
            # שמירת הקובץ בתיקייה המתאימה
            file.save(os.path.join(app.config['UPLOAD_FOLDER'], file.filename))
            # הוספת הודעה עם שם המשתמש ושם הקובץ לרשימת ההודעות
            messages.append({'username': username, 'message': message, 'file': file.filename})
            return 'Message sent successfully'  # הודעת הצלחה
    else:
        # במקרה שלא נשלח קובץ
        messages.append({'username': username, 'message': message, 'file': None})
        return 'Message sent successfully'  # הודעת הצלחה
